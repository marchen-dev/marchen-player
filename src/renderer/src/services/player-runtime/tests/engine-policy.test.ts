...
import { choosePlaybackEngine, normalizeEnginePreference } from '../engine-policy'
...
it.each([
  ...
  [null, 'auto'],
  ['invalid', 'auto'],
])('归一化旧偏好 %s → %s', (input, expected) => {
  expect(normalizeEnginePreference(input)).toBe(expected)
})
...
describe('实际选中轨道的内核选择', () => {
  ...
  it('已知不支持才直接使用 Canvas，未知编码保留实际尝试', async () => {
    ...
    expect(await choosePlaybackEngine('auto', resource(), probe)).toBe('compat')
    probe.mockClear()
  ...
  it('自动模式保留备用音轨，固定内核遵从明确选择', async () => {
    const probe = vi.fn(() => 'probably' as const)
    expect(await choosePlaybackEngine('auto', resource(), probe, 3)).toBe('compat')
    expect(await choosePlaybackEngine('native', resource(), probe, 3)).toBe('native')
    expect(await choosePlaybackEngine('compat', resource(), probe)).toBe('compat')
    expect(probe).not.toHaveBeenCalled()
  })
})
