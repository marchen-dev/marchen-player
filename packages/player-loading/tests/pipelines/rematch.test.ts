/**
 * ready 数据重新匹配 Pipeline 测试
 *
 * 测试重新匹配和本地弹幕后通过 state$ 发布新 ready 数据
 */

import type { LoadingState } from '../../src/types'
import { filter, firstValueFrom, take, timeout } from 'rxjs'
import { afterEach, describe, expect, it } from 'vitest'
import { PlayerLoadingService } from '../../src/service'
import { createMockDeps } from '../helpers/mock-ports'

function waitForStep(service: PlayerLoadingService, step: string): Promise<LoadingState> {
  return firstValueFrom(
    service.state$.pipe(
      filter((s) => s.step === step),
      take(1),
      timeout(3000),
    ),
  )
}

describe('rematch Pipeline', () => {
  let service: PlayerLoadingService

  afterEach(() => {
    service?.destroy()
  })

  it('rematch 应该经过 ready → reloading → ready', async () => {
    const deps = createMockDeps()
    service = new PlayerLoadingService(deps)

    // 先进入 ready 状态
    service.loadFromPath('/test.mkv')
    await waitForStep(service, 'ready')

    // 触发 rematch
    service.rematch({
      episodeId: 2001,
      animeTitle: '新匹配',
      episodeTitle: '第2话',
      animeId: 200,
    })

    // 应该经过 reloading 再回到 ready
    const reloadingState = await waitForStep(service, 'reloading')
    expect(reloadingState.step).toBe('reloading')

    const readyState = await waitForStep(service, 'ready')
    expect(readyState.step).toBe('ready')
    // 弹幕应该已更新
    expect(deps.api.getDanmu).toHaveBeenCalledWith(2001, expect.any(Object))
  })

  it('rematch 应该通过 state$ 发布新的 ready 弹幕数据', async () => {
    const deps = createMockDeps()
    service = new PlayerLoadingService(deps)

    // 先进入 ready
    service.loadFromPath('/test.mkv')
    await waitForStep(service, 'ready')

    // rematch
    service.rematch({
      episodeId: 3001,
      animeTitle: '重新匹配',
      episodeTitle: '第3话',
      animeId: 300,
    })

    await waitForStep(service, 'reloading')
    const state = await waitForStep(service, 'ready')

    expect(state.step).toBe('ready')
    if (state.step === 'ready') {
      expect(state.match.episodeId).toBe(3001)
      expect(state.mergedComments).toHaveLength(3)
    }
  })

  it('非 ready 状态下 rematch 应该被忽略', async () => {
    const deps = createMockDeps()
    service = new PlayerLoadingService(deps)

    // 在 idle 状态下 rematch
    service.rematch({
      episodeId: 1001,
      animeTitle: 'test',
      episodeTitle: 'E1',
      animeId: 100,
    })

    // 状态应该保持 idle
    expect(service.currentState.step).toBe('idle')
  })

  it('addLocalDanmaku 应该追加弹幕并发布新的 ready 数据', async () => {
    const deps = createMockDeps()
    service = new PlayerLoadingService(deps)

    // 先进入 ready
    service.loadFromPath('/test.mkv')
    await waitForStep(service, 'ready')

    // 添加本地弹幕
    await service.addLocalDanmaku({
      type: 'local',
      source: 'test.xml',
      selected: true,
      content: {
        count: 2,
        comments: [
          { cid: 100, m: '本地弹幕1', p: '1.00,1,16777215,local' },
          { cid: 101, m: '本地弹幕2', p: '2.00,1,16777215,local' },
        ],
      },
    })

    const state = service.currentState
    expect(state.step).toBe('ready')
    if (state.step === 'ready') {
      // 弹幕列表应该包含新增的 local 条目
      expect(state.danmaku.length).toBeGreaterThan(1)
      expect(state.danmaku.some((d) => d.source === 'test.xml')).toBe(true)
    }
    expect(deps.cache.set).toHaveBeenCalled()
  })

  it('切换弹幕源后应该重新合并并发布 ready 数据', async () => {
    const deps = createMockDeps()
    service = new PlayerLoadingService(deps)

    service.loadFromPath('/test.mkv')
    await waitForStep(service, 'ready')

    await service.setDanmakuSourceSelected('dandanplay', false)

    const state = service.currentState
    expect(state.step).toBe('ready')
    if (state.step === 'ready') {
      expect(state.danmaku[0]?.selected).toBe(false)
      expect(state.mergedComments).toEqual([])
    }
    expect(deps.cache.set).toHaveBeenCalled()
    expect(deps.history.save).toHaveBeenCalled()
  })
})
